function [ A_new best_err pre_err best_step ] = update_cov_bases(...
...
% Compute the average gradients across all of the observations
A_grads = basis_gradients(A, B(train_idx,:), X(train_idx,:), l1_pen);

% Symmetrize the bases (to account for use as sparse GGM bases)
for i=1:basis_count,
    basis = squeeze(A(:,:,i));
    A(:,:,i) = (basis + basis') ./ 2;
end
...
for i=1:numel(trial_steps),
    ...
    X_hat = covcode_decode(X_test, A_new, B_test);
